...
class ExcelData(login.CommonLogin, styles.FontColor, db_login.DBConnection):
    ...
    def __init__(self):
        ...
        self.candidate_filter = ['CandidateIds', 'Name', 'Email', 'Phone', 'USN', 'CreatedBy', 'PanNo', 'AadhaarNo',
                                 'CurrentLocationIds', 'TypesOfSource', 'SourceId', 'Sourcer', 'Gender', 'MaritalStatus',
                                 'Gender', 'MaritalStatus', 'CollegeIds', 'DegreeIds', 'BranchIds',
                                 'YearEnd', 'ExpertiseIds', 'WorkProfileOrganisationIds', 'Text1', 'Text2', 'Text3', 'Text4',
                                 'Text5', 'Integer1', 'Integer2', 'Integer3', 'Integer4', 'Integer5', 'TextArea1',
                                 'TextArea2', 'TextArea3', 'TextArea4', 'TextArea5', 'DateCustomField1', 'DateCustomField2'
                                 , 'CandidateUtilization', 'ExperienceFrom', 'ExperienceTo', 'PercentageFrom', 'PercentageTo'
                                 , 'CreatedFrom', 'CreatedTo', 'ModifiedFrom', 'ModifiedTo']

        self.applicant_filters = ['JobDepartmentIds', 'JobIds', 'ApplicantStatusIds', 'EventIds']

        self.candidate_custom_filters = ['Text6', 'Text7', 'Text8', 'Text9', 'Text10', 'Text11', 'Text12', 'Text13', 'Text14',
                                         'Text15', 'Integer6', 'Integer7', 'Integer8', 'Integer9', 'Integer10', 'Integer11',
                                         'Integer12', 'Integer13', 'Integer14', 'Integer15', 'DateCustomField3',
                                         'DateCustomField4', 'DateCustomField5']
        self.candidate_test_filter = ["TestIds", "StatusIds", "TestScoreFrom", "TestScoreTo"]

        # self.our_expected_ids =[1219152, 1219153,1219154,1219155,1219156,1219157,1219158,1219159,1219160,1219161,1219162,
        #                         1219163,1219164,1219165,1219166,1219167,1219168,1219169,1219170,1219171,1219172,1219173,
        #                         1219174,1219175,1219176,1219177,1219178,1219179,1219180,1219181,1219182,1219183,1219184,
        #                         1219185,1219186,1219187,1219188,1219189,1219190,1219191,1219192,1219193,1219194,1219195,
        #                         1219196,1219197,1219198,1219199,1219200,1219201]
        ...
        # self.ws.write(0, 3, 'Expected Candidate Id\'s', self.style11)
        # self.ws.write(0, 4, 'Not Matched Id\'s', self.style11)
    ...
    def all(self):
        ...
        for i in range(0,self.tot_len):
            ...
            self.data = {
                "PagingCriteria": {"IsRefresh": False, "IsSpecificToUser": False, "MaxResults": 200, "PageNo": 1,
                                   ...
                                   "IsCountOnly": True}, "CandidateFilters": final_candidate_filter_request,
                                    "CandidateCustomFilters":custom_dictionary,"ApplicantFilters":applicant_dictionary,
                            "CandidateTestFilters":final_candidate_test_filter_request,"IsNotCacheRequired": False}
            # print self.data
            self.json_data()
            ...
            self.ws.write(self.rownum, 1, str(self.xl_request))

            if self.total_api_count == self.Query_Result1:
                self.ws.write(self.rownum, 2, self.total_api_count)
                self.ws.write(self.rownum, 3, self.Query_Result1, self.style14)
                # self.ws.write(self.rownum, 3, expected_id, self.style14)
                # self.ws.write(self.rownum, 4, mismatched_id, self.style13)
                # self.ws.write(self.rownum, 5, str(self.actual_ids), self.style14)
                ...
                # self.ws.write(self.rownum, 3, expected_id, self.style14)
                # self.ws.write(self.rownum, 4, "API Failed", self.style13)
                # self.ws.write(self.rownum, 5, str(self.actual_ids), self.style13)
                print(self.query)
            else:
                ...
                # self.ws.write(self.rownum, 5, str(self.actual_ids), self.style13)
                print(self.query)
            self.wb_result.save(output_paths.outputpaths['candidate_search_output_sheet_2'])
            # print statusCode, " -- ", b
    ...
    def Query_Generation(self):
        ...
        if self.xl_request.get("ModifiedFrom") and self.xl_request.get("ModifiedTo"):
            modefied_on_from = Object.date_converter(self.xl_request.get("ModifiedFrom"))
            modefied_on_to = Object.date_converter(self.xl_request.get("ModifiedTo"))
            where_str += " and date(c.modified_on)  between '%s' and '%s' " %(modefied_on_from,modefied_on_to)

        if self.xl_request.get("Name"):
            where_str += " and hp_dec(c.candidate_name) COLLATE utf8mb4_general_ci like '%{}%' ".format(self.xl_request.get("Name"))
        ...
        if self.xl_request.get("PanNo"):
            where_str += " and hp_dec(c.pan_card) COLLATE utf8mb4_general_ci = '{}'".format(self.xl_request.get("PanNo"))
        if self.xl_request.get("AadhaarNo"):
            where_str += " and hp_dec(c.aadhaar_no) COLLATE utf8mb4_general_ci = {}".format(self.xl_request.get("AadhaarNo"))
    ...
    def overall_status(self):
        self.ws.write(0, 0, 'Candidates search', self.style23)
