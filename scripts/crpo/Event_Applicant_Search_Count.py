...
class Excel_Data(login.CommonLogin, work_book.WorkBook, db_login.DBConnection):
    ...
    """ Enable This Method if you required expected and matched id"""
    # ------------------------------------------------------------------------------------------------------------------
    ...
    def all(self):
        ...
        for i in range(0,tot_len):
            ...
            """ Enable Below Lines if you required expected and matched id"""
            ...
            elif self.Query_Result1 in [2, 6]:
                self.ws.write(self.rownum, 0, 'Pass', self.style26)
            ...
            if self.total_api_count == self.Query_Result1:
                ...
                self.ws.write(self.rownum, 2, self.total_api_count, self.style7)
                ...
                # self.ws.write(self.rownum, 4, mismatched_id, self.style13)
                print(self.query)
            self.wb_result.save(output_paths.outputpaths['Applicant_count_Output_sheet_1'])
            # print statusCode, " -- ", b
    ...
    def Query_Generation(self):
        ...
        where_str = " c.is_deleted=0 and c.is_draft=0 and c.is_archived=0 and c.tenant_id=1787 and sp.is_deleted=0 " \
                    "and sp.recruitevent_id= %s " %event_id
        ...
        if self.xl_request.get("CandidateIds"):
            ...
            values = ','.join(str(v) for v in a)
            # event_id =  self.xl_request.get("eventId")
            where_str += " and sp.candidate_id in ( %s )" %(values)
    ...
    def overall_status(self):
        self.ws.write(0, 0, 'Applicant Search', self.style23)
