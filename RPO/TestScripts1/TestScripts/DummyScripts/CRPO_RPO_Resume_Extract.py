...
class extractResume(unittest.TestCase):
    def test_extractResume(self):
        ...
        ws.write(0, 2, 'Not Extracted Primary Skills', __style0)
        ...
        ws.write(0, 25, 'Extracted Secondary Skills', __style0)
        ws.write(0, 26, 'Not Extracted Secondary Skills', __style0)
        ws.write(0, 27, 'Matched Secondary Skills (Expected vs Extracted)', __style0)

        wb = xlrd.open_workbook("/home/sanjeev/TestScripts/TestScripts/Input_Data/ExtractResumeNew02.xls")
        ...
        while i < sh1.nrows:
            ...
            try:
                if self.driver.find_element_by_xpath("//div[3]/div/create-update-candidate/section/div[1]/div/div[2]/div[8]/"
                                                     "transcluded-input/div/div/div[2]").is_displayed() != 0:
                    ...
                    if not __college:
                        ...
                        if __college == "Others":
                            __collegeOthers = self.driver.find_element_by_xpath(
                                "//div[3]/div/create-update-candidate/section/div[1]/div/div[2]/div[8]"
                                "/transcluded-input/div/div/div[2]/table/tbody/tr/td[1]").get_attribute(
                                "value")
            ...
            #    print("College name is not extracted")
                #ws.write(rownum, 11, "College name is not extracted", __style1)
            try:
                if self.driver.find_element_by_xpath("//div[3]/div/create-update-candidate/section/div[1]/div/div[2]/div[8]"
                                                     "/transcluded-input/div/div/div[2]").is_displayed() != 0:
                    __degree = self.driver.find_element_by_xpath("//div[3]/div/create-update-candidate/section/div[1]/div/div[2]/div[8]"
                                                                 "/transcluded-input/div/div/div[2]/table/tbody/tr/td[2]").text
                    if not __degree:
                ...
                print("Degree Name Not Extracted")
                #ws.write(rownum, 12, "Degree Name is not extracted", __style1)
            ...
            try:
                if self.driver.find_element_by_xpath("//div[3]/div/create-update-candidate/section/div[1]/div/div[2]/div[8]"
                    ...
                    __branch = self.driver.find_element_by_xpath(
                        "//div[3]/div/create-update-candidate/section/div[1]/div/div[2]/div[8]"
                        "/transcluded-input/div/div/div[2]/table/tbody/tr/td[3]").text
                    if not __branch:
                        print ("Branch is not extracted")
                        ws.write(rownum, 13, __branch, __style1)
                    else:
                        if __branch.lower() == branch.lower():
                            print ("Branch is extracted as expected")
                            ws.write(rownum, 13, __branch, __style3)
                        else:
                            print('Branch "%s"' % __branch + ' is not extracted as expected "%s"' % branch)
                            ws.write(rownum, 13, __branch, __style2)

            except NoSuchElementException:
                print("Branch Name Not Extracted")
                #ws.write(rownum, 13, "Branch Name is not extracted", __style1)
            try:
                if self.driver.find_element_by_xpath("//div[3]/div/create-update-candidate/section/div[1]/div/div[2]/div[8]"
                                                     "/transcluded-input/div/div/div[2]").is_displayed() != 0:
                    __yop = self.driver.find_element_by_xpath(
                        "//div[3]/div/create-update-candidate/section/div[1]/div/div[2]/div[8]"
                        "/transcluded-input/div/div/div[2]/table/tbody/tr/td[4]").text
                    if not __yop:
                        ...
                        if int(__yop) == int(yop):
                            ...
                            print(
                            'YOP "%s"' % (str(int(__yop))) + ' is not extracted as expected "%s"' % (str(int(yop))))
                            ws.write(rownum, 14, __yop, __style2)

            except NoSuchElementException:
                print("YOP Is Not Extracted")
                #ws.write(rownum, 14, "YOP is not extracted", __style1)
            try:
                if self.driver.find_element_by_xpath("//div[3]/div/create-update-candidate/section/div[1]/div/div[2]/div[8]"
                                                     "/transcluded-input/div/div/div[2]").is_displayed() != 0:
                    __percentageorcgpa = self.driver.find_element_by_xpath(
                        "//div[3]/div/create-update-candidate/section/div[1]/div/div[2]/div[8]"
                        "/transcluded-input/div/div/div[2]/table/tbody/tr/td[5]").text
                    if not __percentageorcgpa:
                        ...
                        if __percentageorcgpa == cgpaorpercentage:
                            ...
                            print(
                            'Percentage/CGPA "%s"' % __percentageorcgpa + ' is not extracted as expected "%s"' % str(
                                int(cgpaorpercentage)))
                            ws.write(rownum, 15, __percentageorcgpa, __style2)

            except NoSuchElementException:
                print("Percentage/CGPA Is Not Extracted")
                #ws.write(rownum, 15, "Percentage/CGPA is not extracted", __style1)
            ...
            __primarySkillsOfCandidate = self.driver.find_element_by_xpath("//div[3]/div/create-update-candidate/section"
                                                                           "/div[1]/div/div[2]/div[2]/transcluded-input/div/div/div[1]"
                                                                           "/div/div/span/span[1]").text
            __primaryExtractedSkills = __primarySkillsOfCandidate.replace("\nRemove\nPress delete to remove this chip.", ",")
            __primaryExtracted_Skills1 = [s.strip() for s in __primaryExtractedSkills]
            __primaryExtractedSkills = [s.strip().lower() for s in __primaryExtractedSkills.split(',')]
            __excel_Skills = rows[2]
            __excel_Skills = [x.strip().lower() for x in __excel_Skills.split(',')]
            __not_matched = []
            __matched = []
            for skill in __excel_Skills:
                if len(skill) > 0:
                    if skill not in __primaryExtractedSkills:
                        __not_matched.append(skill + ", ")
                    else:
                        __matched.append(skill + ", ")
                else:
                    print("All Primary Skills Matched")
            ws.write(rownum, 1, __primaryExtracted_Skills1, __style1)
            ws.write(rownum, 2, __not_matched, __style1)
            ws.write(rownum, 16, __matched, __style1)

            __secondarySkillsOfCandidate = self.driver.find_element_by_xpath("//div[3]/div/create-update-candidate/section"
                                                                             "/div[1]/div/div[2]/div[2]/transcluded-input/div/div"
                                                                             "/div[2]/div/div/span/span[1]").text
            __secondaryExtractedSkills = __secondarySkillsOfCandidate.replace("\nRemove\nPress delete to remove this chip.", ",")
            __secondaryExtracted_Skills1 = [s.strip() for s in __secondaryExtractedSkills]
            __secondaryExtractedSkills = [s.strip().lower() for s in __secondaryExtractedSkills.split(',')]
            __excel_Skills = rows[2]
            __excel_Skills = [x.strip().lower() for x in __excel_Skills.split(',')]
            __not_matched = []
            __matched = []
            for skill in __excel_Skills:
                if len(skill) > 0:
                    if skill not in __secondaryExtractedSkills:
                        __not_matched.append(skill + ", ")
                    else:
                        __matched.append(skill + ", ")
                else:
                    print("All Secondary Skills Matched")
            ws.write(rownum, 25, __secondaryExtracted_Skills1, __style1)
            ws.write(rownum, 26, __not_matched, __style1)
            ws.write(rownum, 27, __matched, __style1)
