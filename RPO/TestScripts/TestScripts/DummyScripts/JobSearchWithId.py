...
class Excel_Data:
    def __init__(self):
        ...
        self.boundary_range = [28590,28589,28588,28587,28586,28585,28584,28583,28582,28581,28580,28579,28578,
                               28577,28576,28575,28574,28573,28572,28571,28570,28569,28568,28567,28566,28565,
                               28564,28563,28562,28561,28560,28559,28558,28557,28556,28555,28554,28553,28552,
                               28551,28550,28549,28548,28547,28546,28545,28544,28543,28542,28541,28540,28539,
                               28538,28537,28536,28535,28534,28533,28532,28531,28530,28529,28528,28527,28526,
                               28525,28524,28523,28522,28521,28520,28519,28518,28517,28516,28515,28514,28513,
                               28512,28511,28510,28509,28508,28507,28506,28505,28504,28503,28502,28501,28500,
                               28499,28498,28497,28496,28495,28494,28493,28492,28491,28429,28427,28389,28368,
                               28362,28360,28389]
        ...
        wb = xlrd.open_workbook('C:\PythonAutomation\InputForJobOfferResumeSearch\JobSearchWithIdsInAutoTenant.xls')
    ...
    def all(self):
        ...
        for i in range(0, tot_len):
            ...
            if self.total_api_count == self.Query_Result1:
                ...
                self.ws.write(self.rownum, 4, mismatched_id, self.__style2)
            self.wb_result.save(
                'C:\PythonAutomation\JobSearchResults/'
                + self.__current_DateTime + '_Combined_Job_Search.xls')
            # print statusCode, " -- ", b
    ...
    def Query_Generation(self):
        ...
        where_str = "j.is_deleted=0 and j.is_archived=0 and j.tenant_id=1793 and j.id in ( %s ) "% values
